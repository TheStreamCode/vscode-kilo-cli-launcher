...
Current release hardening focuses on predictable terminal launch behavior, workspace-aware terminal placement, an interactive terminal install prompt when the default `kilo` command is missing, npm-based local tooling, packaged branding assets for VS Code and Marketplace surfaces, and automated validation in CI.
